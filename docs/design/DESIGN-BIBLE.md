...
9. **C2 — The character is a conditionally-transferable asset with one cash-out.** Transfer-locked while a season is open / tasks unfinished; unlocks on season-completion. Selling a season-complete character is the **single value-exit**; buyer inherits the collection, not stats; seller restarts at zero; one character per account per season. The prototype account is the browser-local game credential; any production **sale-capable** season instead binds that cap to one verified identity per season, with the wallet serving only as the payment/settlement rail. *Production go-live is gated on legal/compliance sign-off — a hard precondition.*
